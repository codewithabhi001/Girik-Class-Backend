...
export const schemas = {
    ...
    submitSurvey: Joi.object({
        ...
        survey_statement: Joi.string().optional().allow('', null).max(5000),
    ...
    createClient: Joi.object({
        ...
        company_id_number: Joi.string().optional().allow('', null).max(50),
        email: Joi.string().email().required(),
        address: Joi.string().required().max(500),
        country: Joi.string().required().max(100),
        phone: Joi.string().pattern(/^(\+?\d{1,4}[\s-]?)?\d{6,15}$/).message('Invalid phone number').required(),
        contact_person_name: Joi.string().required().max(100),
        contact_person_email: Joi.string().email().required(),
        status: Joi.string().valid('ACTIVE', 'INACTIVE').optional(),
        user: Joi.object({
            name: Joi.string().required(),
            email: Joi.string().email().required(),
    ...
    createSurveyor: Joi.object({
        ...
        password: Joi.string().min(8).pattern(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/).required().messages({ 'string.pattern.base': 'Password must contain uppercase, lowercase, and digit' }),
        phone: Joi.string().pattern(/^(\+?\d{1,4}[\s-]?)?\d{6,15}$/).message('Invalid phone number').required(),
        license_number: Joi.string().optional().allow('', null).max(50),
        authorized_ship_types: Joi.array().items(Joi.string()).optional(),
        authorized_certificates: Joi.array().items(Joi.string()).optional(),
        valid_from: Joi.date().iso().optional(),
    ...
    createVessel: Joi.object({
        ...
        port_of_registry: Joi.string().required().max(100),
        ...
        current_class_society: Joi.string().optional().allow('', null).max(100),
        engine_type: Joi.string().optional().allow('', null).max(100),
        builder_name: Joi.string().optional().allow('', null).max(100),
        uploaded_documents: Joi.array().items(Joi.object({
            file_url: Joi.string().required(),
            document_type: Joi.string().required(),
            description: Joi.string().optional().allow(''),
    ...
    updateVessel: Joi.object({
        ...
        port_of_registry: Joi.string().optional().max(100),
        year_built: Joi.number().integer().optional(),

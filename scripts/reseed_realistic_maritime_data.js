...
const TABLES_TO_TRUNCATE = [
    ...
    'clients', 'vessels', 'flag_administrations'
];
...
async function syncRequiredDocuments(typeMap) {
    ...
    const existingTypes = await db.CertificateType.findAll({ useMaster: true });
    ...
    for (const row of rows) {
        ...
        const [reqDoc, created] = await db.CertificateRequiredDocument.findOrCreate({
            where: {
...
const main = async () => {
    try {
        ...
        // 1. Delete Client User Accounts
        console.log('👤 Deleting client-role user accounts...');
        const clientUsers = await db.User.findAll({ where: { role: 'CLIENT' }, attributes: ['id'] });
        const clientUserIds = clientUsers.map(u => u.id);
        if (clientUserIds.length > 0) {
            await db.NotificationPreference.destroy({ where: { user_id: clientUserIds } });
        }
        const clientUsersDeleted = await db.User.destroy({ where: { role: 'CLIENT' } });
        console.log(`   Deleted ${clientUsersDeleted} CLIENT users.`);

        // 2. Clear Database (Truncating specified tables)
        console.log('🧹 Truncating tables...');

        for (const table of TABLES_TO_TRUNCATE) {
            try {
                const quoted = dialect === 'mysql' ? `\`${table}\`` : `"${table}"`;
                await db.sequelize.query(`TRUNCATE TABLE ${quoted}`);
        ...
        for (const f of flagsData) {
            const flag = await db.FlagAdministration.create(f);
            flags.push(flag);
            console.log(`   Created Flag: ${flag.flag_state_name}`);
        }
        ...
        for (const v of vesselsData) {
            const vessel = await db.Vessel.create(v);
            console.log(`   Created Vessel: ${vessel.vessel_name} (IMO: ${vessel.imo_number})`);
        }
        ...
        // 8. Seeding HTML Certificate Templates
        ...
        for (const certFolder of sortedFolders) {
            ...
            // Process docx template files if they exist in repo
            if (folderPath && fs.existsSync(folderPath)) {
                ...
                for (const docxFile of docxFiles) {
                    ...
                    try {
                        ...
                        console.error(`   - Failed to upload checklist ${path.basename(docxFile)}: ${uploadErr.message}`);
                    }
                }
            }

            // Derive realistic checklist questions
            ...
            // Create checklist files association
            ...
            checklistCount++;
            console.log(`   - Created checklist for: "${matchedType.name}"`);
        }
        ...
        console.log('\n✨ Database Cleanup & Seed completed successfully! ✨');
        process.exit(0);
    } catch (err) {
        console.error('❌ Database Cleanup & Seed failed:', err);
        process.exit(1);
    }
};

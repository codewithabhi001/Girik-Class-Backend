...
export const finalizeSurvey = async (req, res, next) => {
    try {
        const result = await surveyService.finalizeSurvey(req.params.id, req.user.id);
        res.json({
            success: true,
...
export const streamLocation = async (req, res, next) => {
    try {
        const result = await surveyService.streamLocation(req.params.id, req.body, req.user.id);
        res.json({
            success: true,
...
export const uploadProof = async (req, res, next) => {
    try {
        const result = await surveyService.uploadProof(req.params.id, req.file, req.user.id);
        res.json({
            success: true,
...
export const flagViolation = async (req, res, next) => {
    try {
        const result = await surveyService.flagViolation(req.params.id, req.user.id);
        res.json({
            success: true,
